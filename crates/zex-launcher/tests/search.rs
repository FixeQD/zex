...
fn bang_trigger_is_web_search() {
    assert_eq!(
        ...
        Intent::Web { trigger: "gh".into(), query: "rust combinators".into() }
    );
    assert_eq!(
        detect("!wiki berlin"),
        Intent::Web { trigger: "wiki".into(), query: "berlin".into() }
    );
}
...
fn arithmetic_is_calculated() {
    assert_eq!(
        detect("2+2"),
        Intent::Math { expression: "2+2".into(), answer: Some("4".into()) }
    );
    assert_eq!(
        detect("17 * 3 - 20%"),
        Intent::Math { expression: "17 * 3 - 20%".into(), answer: Some("50.8".into()) }
    );
}

#[test]
fn prose_with_numbers_is_not_math() {
    assert_eq!(detect("top 10 movies"), Intent::Browse("top 10 movies".into()));
